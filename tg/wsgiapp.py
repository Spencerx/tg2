...
class TGApp(object):
    ...
    def __call__(self, environ, start_response):
        ...
        try:
            ...
            raise Exception(
                "No content returned by controller (Did you "
                "remember to 'return' it?) in: %r" % controller
            )
        finally:
    ...
    def _setup_app_env(self, environ):
        ...
        locals.session = environ.get(
            "beaker.session"
        )  # Usually None, unless middleware in place
    ...
    def lookup_controller(cls, config, controller):
        ...
        if base_controller_path is None:
            raise TGConfigError(
                "Unable to load controllers, no controllers path configured!"
            )
